...
namespace BuildTools.MSBuildTasks.UnitTests
{
  ...
  public class BuildTestOutputFilesTest
  {
    ...
    [Test]
    public void MissingExecutionRuntime_LogsError_False ()
    {
      var taskItem = new TaskItem ("MyTest.dll");
      const string config = "SqlServer2014+x64+release+Chrome+net45";
      taskItem.SetMetadata ("TestingConfiguration", config);
      var loggerMock = MockRepository.Mock<ITaskLogger>();
      loggerMock.Expect (_ => _.LogError ("{0} ('{1}')", "Could not find a supported execution runtime.", config));
      var items = new ITaskItem[] { taskItem };
      var task = CreateBuildTestOutputFiles (items, logger: loggerMock);
    ...
    [Test]
    public void MissingPlatform_LogsError_False ()
    {
      var taskItem = new TaskItem ("MyTest.dll");
      const string config = "SqlServer2014+Win_NET46+release+Chrome+net45";
      taskItem.SetMetadata ("TestingConfiguration", config);
      var loggerMock = MockRepository.Mock<ITaskLogger>();
      loggerMock.Expect (_ => _.LogError ("{0} ('{1}')", "Could not find a supported platform.", config));
      var items = new ITaskItem[] { taskItem };
      var task = CreateBuildTestOutputFiles (items, logger: loggerMock);
    ...
    [Test]
    public void MissingConfigurationID_LogsError_False ()
    {
      var taskItem = new TaskItem ("MyTest.dll");
      const string config = "SqlServer2014+x64+Win_NET46+Chrome+net45";
      taskItem.SetMetadata ("TestingConfiguration", config);
      var loggerMock = MockRepository.Mock<ITaskLogger>();
      loggerMock.Expect (_ => _.LogError ("{0} ('{1}')", "Could not find a supported configuration ID.", config));
      var items = new ITaskItem[] { taskItem };
      var task = CreateBuildTestOutputFiles (items, logger: loggerMock);
    ...
    [Test]
    public void IncludeCategories_NotProvided_Semicolon ()
    {
      var taskItem = new TaskItem ("C:\\Path\\To\\MyTest.dll");
      const string config = "release+SqlServer2014+x64+Win_NET46+Chrome+net45";
      taskItem.SetMetadata ("TestingConfiguration", config);
      ...
      Assert.That (task.Output.Single().GetMetadata (TestingConfigurationMetadata.IncludeCategories), Is.EqualTo (";"));
    }
